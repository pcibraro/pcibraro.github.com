---
layout: post
title: "Doing DI with Autofac in ASP.NET Web API"
date: 2012-02-29
comments: true
categories: .NET
---
...
GetService should return null if the dependency can not resolved.
...
The third overload receives a set of Func delegates for doing the same
thing.
...
with the one required by ASP.NET Web API.

The AutofacDepedencyResolver class in that package implements
...
The following code snippet illustrates how you can register all
...
One the things you can do with Autofac is to set the lifetime of the
...
existing Api controllers in the the project into the DI container.
...
Using the code above, you should be able to use a API controller that
looks like this (The contact repository is injected as a depedency)
