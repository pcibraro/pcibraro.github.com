---
layout: post
title: "Exposing additional service metadata with WS-Discovery in WCF 4.0"
date: 2010-09-17
comments: true
categories: .NET
---

WS-Discovery is not only a mechanism for discovering service endpoint
...
clients could use at any time.

With all this, I am not saying that WS-Discovery is a good replacement
...
However, they both complement very well each, the service can configure
itself from the service repository, and expose metadata and runtime
information to clients through WS-Discovery.

There are two types of metadata attributes or extensions that you can
configure in the “endpoint” discovery behavior,
...
Scopes, which represents “URI”s and can be used by clients to filter the
...
provides a “urn:CRM” scope, so this service will match that probe.

On the client side, the WCF client can specify the “scopes” as part of
the “FindCriteria” argument passed to the DiscoveryClient,
...
Then, you have “extensions”, which are xml elements that provide
...
to any service metadata or documentation.  

The service metadata endpoint (or MEX endpoint in WCF) is useful here as
part of the extensions because WS-Discovery only returns the "service”
endpoint address as part of the response for the probe messages.

The extensions are available on the client side as part of the discovery
response message in the found endpoints.
